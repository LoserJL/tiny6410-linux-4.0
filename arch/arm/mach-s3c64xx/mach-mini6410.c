...
void s3c_otg_phy_config(int enable)
{
	...
	//use EPLL 48MHz
	//__raw_writel((__raw_readl(S3C_CLK_SRC) | (1<<2) | (1<<5)), S3C_CLK_SRC);
	__raw_writel((__raw_readl(S3C_CLK_SRC) | (2<<5)), S3C_CLK_SRC);

	//divider ratio
	//__raw_writel((__raw_readl(S3C_CLK_DIV1) | (0<<20)), S3C_CLK_DIV1);
	__raw_writel((__raw_readl(S3C_CLK_DIV1) | (0<<20)), S3C_CLK_DIV1);
...
//enable sdhci clock    --by JL
void s3c_sdhci_clk_enable(void)
{
	//use external 27MHz
	__raw_writel((__raw_readl(S3C_CLK_SRC) | (3<<18) | (3<<20)), S3C_CLK_SRC);

	//divider ratio
	//__raw_writel((__raw_readl(S3C_CLK_DIV1) | (0<<0) | (0<<4)), S3C_CLK_DIV1);
}
