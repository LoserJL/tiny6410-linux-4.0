...
#define S3C2410_INT_CSTAT	      S3C_TIMERREG(0x44)
#define TIMER2_ENABLE				(1<<2)
#define TIMER2_CLEAR				(1<<7)
...
static irqreturn_t timer_for_1wire_interrupt(int irq, void *dev_id)
{
	...
	writel(readl(S3C2410_INT_CSTAT) | TIMER2_CLEAR, S3C2410_INT_CSTAT);
	return IRQ_HANDLED;
}
...
static void start_one_wire_session(unsigned char req)
{
	...
	writel(TCNT_FOR_SAMPLE_BIT, S3C2410_TCNTB(2));
	...
	writel(readl(S3C2410_INT_CSTAT) | TIMER2_ENABLE | TIMER2_CLEAR, S3C2410_INT_CSTAT);
}
...
static int __init dev_init(void)
{
	...
	if (ret == 0) {
		setup_irq(IRQ_TIMER2_VIC, &timer_for_1wire_irq);
		...
		/*proc_create("driver/one-wire-info", 0, NULL, &one_fops);*/
	}
...
static void __exit dev_exit(void)
{
	...
	free_irq(IRQ_TIMER2_VIC, &timer_for_1wire_irq);
