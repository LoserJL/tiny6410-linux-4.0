...
static int s3cfb_set_par(struct fb_info *info)
{
	...
	fbi->fb.fix.line_length = var->xres * s3cfb_fimd.bytes_per_pixel;
...
static void s3cfb_init_fbinfo(s3cfb_info_t *finfo, char *drv_name, int index)
{
	...
	finfo->fb.fix.line_length = finfo->fb.var.xres * s3cfb_fimd.bytes_per_pixel;
...
static int __init s3cfb_probe(struct platform_device *pdev)
{
	...
	//clk_enable(info->clk);
	clk_prepare_enable(info->clk); //modify by JL
